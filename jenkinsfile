    ...
    stages {
        ...
        stage('Deploy to AWS Ubuntu Server') {
            steps {
                ...
                sh '''
                scp -o StrictHostKeyChecking=no -r ./ ubuntu@16.171.44.156:/var/www/myapp
                ssh ubuntu@16.171.44.156 'pm2 restart myapp || pm2 start /var/www/myapp/index.js --name myapp'
                '''
            }
        }
    }
